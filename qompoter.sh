...
# It uses CMake FetchContent to declare all dependencies
...
# FetchContent declarations for each dependency
EOF
}
...
prepareVendorDir()
{
  ...
  local vendorCmakeFilepath=${vendorDir}/vendor.cmake.tmp
  ...
  if [ "${IS_NO_QOMPOTE}" == "0" ]; then
    ...
    createVendorCmake "${vendorCmakeFilepath}"
  fi
}
...
updateVendorCmake()
{
  ...
  # Check if package has CMakeLists.txt (required for building)
  if [ ! -f "${packageDir}/CMakeLists.txt" ]; then
    logTrace "no CMakeLists.txt found for package ${packageFullName}, skipping CMake integration"
    return
  fi

  # Generate FetchContent_Declare for this package
  cat << EOF >> "${vendorCmakeFilepath}"

## Start ${packageFullName} ##
FetchContent_Declare(
  ${packageName}
  SOURCE_DIR "\${QOMPOTER_VENDOR_DIR}/${packageName}"
)
## End ${packageFullName} ##
EOF
}
...
finalizeVendorCmake()
{
  ...
  # Add FetchContent_MakeAvailable call for all declared packages
  cat << 'EOF' >> "${vendorCmakeFilepath}"

# Make all dependencies available
set(QOMPOTER_PACKAGES)
EOF
...
if(QOMPOTER_PACKAGES)
  FetchContent_MakeAvailable(${QOMPOTER_PACKAGES})
endif()
EOF
}
...
downloadPackage()
{
  ...
  # DONE
    ...
    updateVendorCmake "${vendorName}" "${packageName}" "${vendorDir}"
...
installAction()
{
  ...
  local vendorDir=$2
  local vendorPriFile
  local vendorCmakeFile
  local globalRes=0

  qompoterLockFile="${qompoterFile/.json/.lock}"
  vendorPriFile=${vendorDir}/vendor.pri
  vendorCmakeFile=${vendorDir}/vendor.cmake
...
updateOneAction()
{
  ...
  if [ -f "${vendorCmakeFile}" ]; then
    cp "${vendorCmakeFile}" "${vendorCmakeFile}.tmp"
  fi
...
refreshVendorCmakeFromQompoterLock()
{
  local qompoterLockFile=$1
  local vendorDir=$2
  local globalRes=0
...
refreshVendorCmakeAction()
{
  ...
  if [[ "${IS_DRYRUN}" == "1" ]]; then
    ...
    if [[ "$?" == 1 ]]; then
      ...
      echo "No change to be expected on ${vendorCmakeFile}"
    fi
    echo
    rm "${vendorCmakeFile}.tmp"
  else
    if [[ "${globalRes}" == 0 ]] || [[ "${IS_BYPASS}" == "1" ]]; then
      ...
      if [[ "$?" == 1 ]]; then
        ...
        echo "No change on ${vendorCmakeFile}"
      fi
      echo
      mv "${vendorCmakeFile}.tmp" "${vendorCmakeFile}"
    else
...
buildAction()
{
  ...
  if [ -z "${installPrefix}" ]; then
    installPrefix="$(cd "${vendorDir}" && pwd)/install"
  fi
  ...
  if [ ! -d "${vendorDir}" ]; then
    echo "Vendor directory '${vendorDir}' does not exist. Run 'qompoter update' first."
    return 100
  fi
  
  echo "Building dependencies with install prefix: ${installPrefix}"
  echo
  
  # Get list of packages from lock file
  local requires
  requires=$(getProjectRequiresFromLock "${qompoterLockFile}")
  
  # Build packages in dependency order (depth-first)
  # We'll process them in the order they appear, as qompoter already handles dependency resolution
  for packageInfo in ${requires}; do
    local vendorName
    local packageName
    vendorName=$(echo "${packageInfo}" | cut -d'/' -f1)
    ...
    echo "* Building ${vendorName}/${packageName}"
    ...
    local buildDir=${packageDir}/build
    mkdir -p "${buildDir}"
    
    # Configure with CMake
    echo "  Configuring..."
    (cd "${buildDir}" && cmake .. -DCMAKE_INSTALL_PREFIX="${installPrefix}" -DCMAKE_PREFIX_PATH="${installPrefix}" > cmake_config.log 2>&1)
    if [ $? -ne 0 ]; then
      echo "  ${C_FAIL}Configuration failed${C_END}. Check ${buildDir}/cmake_config.log"
      if [ "${IS_BYPASS}" == "0" ]; then
        globalRes=1
        return 1
      fi
    ...
    # Build
    echo "  Building..."
    (cd "${buildDir}" && cmake --build . > cmake_build.log 2>&1)
    if [ $? -ne 0 ]; then
      echo "  ${C_FAIL}Build failed${C_END}. Check ${buildDir}/cmake_build.log"
      if [ "${IS_BYPASS}" == "0" ]; then
        globalRes=1
        return 1
      fi
    ...
    # Install
    echo "  Installing..."
    (cd "${buildDir}" && cmake --install . > cmake_install.log 2>&1)
    if [ $? -ne 0 ]; then
      echo "  ${C_FAIL}Installation failed${C_END}. Check ${buildDir}/cmake_install.log"
      if [ "${IS_BYPASS}" == "0" ]; then
        globalRes=1
        return 1
      fi
  ...
  if [ "${globalRes}" == 0 ]; then
    echo "All dependencies built and installed successfully to: ${installPrefix}"
  fi
...
convertAction()
{
  ...
  # Generate CMakeLists.txt with find_package calls based on qompoter.lock
  local qompoterLockFile="${projectDir}/qompoter.lock"
  
  if [ -f "${qompoterLockFile}" ]; then
    echo "Generating CMakeLists.txt with dependency information..."
    
    local cmakeFile="${projectDir}/CMakeLists.qompoter.txt"
    local requires
    requires=$(getProjectRequiresFromLock "${qompoterLockFile}")
    
    if [ "${dryRun}" == "1" ]; then
      echo "  [DRY RUN] Would generate ${cmakeFile} with find_package calls"
      if [ ! -z "${requires}" ]; then
        for packageInfo in ${requires}; do
          local packageName
          packageName=$(echo "${packageInfo}" | cut -d'/' -f2)
          echo "    - find_package(${packageName})"
        done
      fi
    else
      # Create CMakeLists template with dependencies
      cat > "${cmakeFile}" << 'EOF'
# This file is auto-generated by qompoter convert
# It contains find_package() calls for all dependencies from qompoter.lock
# 
# Usage: Copy the relevant sections into your main CMakeLists.txt after running qmake2cmake
# Or include this file: include(CMakeLists.qompoter.txt)

# Include vendor.cmake for FetchContent declarations
if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/vendor/vendor.cmake")
  include(vendor/vendor.cmake)
endif()

# Find all qompoter dependencies
EOF
      ...
      if [ ! -z "${requires}" ]; then
        for packageInfo in ${requires}; do
          ...
          echo "find_package(${packageName} QUIET)" >> "${cmakeFile}"
        done
        
        echo "" >> "${cmakeFile}"
        echo "# Link against qompoter dependencies in your targets:" >> "${cmakeFile}"
        echo "# target_link_libraries(your_target PRIVATE" >> "${cmakeFile}"
        
        for packageInfo in ${requires}; do
          local packageName
          packageName=$(echo "${packageInfo}" | cut -d'/' -f2)
          echo "#   ${packageName}" >> "${cmakeFile}"
        done
        
        echo "# )" >> "${cmakeFile}"
      fi
      
      echo "  Generated ${cmakeFile}"
      echo "  This file contains find_package() calls for your dependencies"
    fi
  else
    echo "No qompoter.lock found, skipping CMakeLists.txt generation"
  fi
  echo
  ...
  if [ "${dryRun}" == "1" ]; then
    ...
    echo "2. Review CMakeLists.qompoter.txt for dependency find_package() calls"
    echo "3. Run qmake2cmake to convert to CMake"
    echo "4. Integrate CMakeLists.qompoter.txt into your generated CMakeLists.txt"
    echo "5. Test the generated CMakeLists.txt files"
    echo ""
...
help()
{
	...
	Options:
	        ...
	        --by-pass         By-pass error and continue the process
	                          Supported actions are: add, build, export --repo, install,
	                          update, updateOne
	    ...
	    -f, --force           By-pass error by forcing the action to be taken
	        ...
	        --no-qompote      Do not generate any Qompoter specific stuffs
	                          like qompote.pri, vendor.pri and vendor.cmake [default = false]
	...
	Examples:
	    ...
	    Reload the vendor/vendor.cmake file from all the available vendor/<package>/cmake/ directories
	      $C_PROGNAME refresh-vendor-cmake
...
main()
{
  ...
  case ${ACTION} in
    ...
    "refresh-vendor-cmake")
      ...
      echo
      refreshVendorCmakeAction "${QOMPOTER_FILENAME}" "${VENDOR_DIR}"
      ;;
    "require")
      echo "======== ${ACTION}"
      echo

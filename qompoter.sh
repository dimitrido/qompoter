...
# Add each package's cmake directory to CMAKE_PREFIX_PATH if it exists
EOF
}
...
updateVendorCmake()
{
  ...
  local cmakeDir=${packageDir}/cmake
  ...
  if [ ! -d "${cmakeDir}" ]; then
    logTrace "no 'cmake' directory found for package ${packageFullName}, skipping CMake integration"
    return
  fi

  # Add CMake prefix path for this package
  { echo ""; echo "## Start ${packageFullName} ##"; echo "list(APPEND CMAKE_PREFIX_PATH \"\${QOMPOTER_VENDOR_DIR}/${packageName}\")"; echo "## End ${packageFullName} ##"; } >> "${vendorCmakeFilepath}"
}
...
help()
{
	...
	Usage: $C_PROGNAME [action] [ --repo <repo> | other options ]
	    ...
	    action               Select an action:
	                          add, export, init, inspect, install, refresh-vendor-cmake, refresh-vendor-pri, show
	                          update, updateOne
